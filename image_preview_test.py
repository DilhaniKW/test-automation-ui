"""Image-preview test runner and helpers.

This module contains a Playwright-based script to exercise image-preview
workflows and helpers for writing test results to `execution_results.csv`.

It also includes a minimal, safe pytest check that verifies the presence of
`execution_results.csv` so CI can validate that results are produced or
preserved between runs. The heavier end-to-end flow is executed via
`run_test()` when this script is run directly.
"""
...
DEFAULT_URL = "https://www.pixelssuite.com/convert-to-png"  # Updated for Assignment 1 - Option 2
...
def test_execution_results_csv_present():
    """Lightweight pytest check: execution_results.csv must exist.

    This test is intentionally small and safe: it only asserts that a
    `execution_results.csv` file exists in the repository root. It does not
    launch a browser or run the full Playwright flow.
    """
    csv_path = Path("execution_results.csv")
    assert csv_path.exists(), "execution_results.csv is missing"